...
2. Create a Hugging Face fine-grained token with **Inference Providers** permission at [Hugging Face Tokens](https://huggingface.co/settings/tokens), then set it in `.env.local`:
   ...
   The default model is `Qwen/Qwen2.5-VL-3B-Instruct`. To choose another compatible vision model, set `HUGGINGFACE_OCR_MODEL`. If your account requires a specific inference provider, use its Hugging Face model identifier with the provider suffix.
...
| `HUGGINGFACE_OCR_MODEL` | No | Vision model used for receipt transcription. Defaults to `Qwen/Qwen2.5-VL-3B-Instruct`. |
...
- Clear, upright receipt photos give the model the most reliable item extraction. Users can correct extracted items before splitting the bill.
